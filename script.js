  ...
  arrow.addEventListener("click", ()=>{
      ...
      if(itemNumber-(4+clickCounter) +(4-ratio>=0)){
         ...
         movieList[i].computedStyleMap().get("transform")[0].x.value - 300
                  ...
                  else {  movieList[i].style.transform = "translateX(0)";
clickCounter=0;}

    })
    });
const ball=document.querySelector(".toggle-ball");
const items=document.querySelectorAll(".container,  .movie-list-title,.navbar-container,.sidebar,.left-menu-icon,.toggle ,.movie-list-wrapper,.profile-container");
